...
class SystemSetting extends Form
{
    ...
    public function form()
    {
        $this->tab(admin_trans('system-setting.labels.base_setting'), function () {
            ...
            $this->switch('is_open_wenzhang', admin_trans('文章首页调用开关。默认开启'))
                ->default(BaseModel::STATUS_OPEN);
                
           $this->switch('is_open_spwz', admin_trans('商品页随机文章开关，默认关闭'))
                ->default(BaseModel::STATUS_CLOSE);
          ...
          $this  ->tab(admin_trans('下单设置'), function() {
               ...
               ->required();
                ...
                ->default(BaseModel::STATUS_OPEN);
                 ...
                 $this->switch('is_buy_login', admin_trans('开启登录购买限制'))
                ->default(BaseModel::STATUS_CLOSE);
            });
            
            $this->tab(admin_trans('商品排序规则'), function() {
               $this->text('grade_1', admin_trans('如果设置price是价格排序，其他则是权重排序'))
             ->default('price') // 注意这里的 'price' 已被正确地放在引号内
              ->required();
            });
           
               $this->tab(admin_trans('商品代理价折扣设置'), function() {
               $this->text('grade_1', admin_trans('代理1级折扣，如果设置为0，不会生成打折价'))
             ->default('0') // 注意这里的 'price' 已被正确地放在引号内
              ->required();
                $this->text('grade_2', admin_trans('代理2级折扣，如果设置为0，不会生成打折价'))
             ->default('0') // 注意这里的 'price' 已被正确地放在引号内
              ->required();
                 $this->text('grade_3', admin_trans('代理3级折扣，如果设置为0，不会生成打折价'))
             ->default('0') // 注意这里的 'price' 已被正确地放在引号内
              ->required();
            });
