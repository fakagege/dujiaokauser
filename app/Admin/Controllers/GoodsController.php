    ...
    protected function form()
    {
        return Form::make(new Goods(), function (Form $form) {
            ...
            $form->saving(function (Form $form) use (&$goodInfo) {
             ...
             // 判断是否是新增操作
   if ($form->isCreating() && $form->actual_price) {
    $actual_price = $form->actual_price;

    // 动态获取折扣比例，确保只有当配置文件中有值并且不为 0 时才使用折扣
    $grade_1_percentage = dujiaoka_config_get('grade_1');
    $grade_2_percentage = dujiaoka_config_get('grade_2');
    $grade_3_percentage = dujiaoka_config_get('grade_3');

    // 如果 grade_1、grade_2、grade_3 为空且折扣比例存在且不为 0 时，动态生成折扣价格
    if (empty($form->grade_1) && !empty($grade_1_percentage) && $grade_1_percentage > 0) {
        $form->grade_1 = $actual_price * ($grade_1_percentage / 10); // 按比例计算折扣，7折即乘以0.7
    }
    if (empty($form->grade_2) && !empty($grade_2_percentage) && $grade_2_percentage > 0) {
        $form->grade_2 = $actual_price * ($grade_2_percentage / 10); // 按比例计算折扣，8折即乘以0.8
    }
    if (empty($form->grade_3) && !empty($grade_3_percentage) && $grade_3_percentage > 0) {
        $form->grade_3 = $actual_price * ($grade_3_percentage / 10); // 按比例计算折扣，9折即乘以0.9
    }
}
...
if ($form->isEditing()) {
    ...
    if ($new_price != $old_price) {
        ...
        // 重新计算代理价格
        if (!empty($grade_1_percentage) && $grade_1_percentage > 0) {
            $form->grade_1 = $new_price * ($grade_1_percentage / 10); // 按比例计算折扣，7折即乘以0.7
        }
        if (!empty($grade_2_percentage) && $grade_2_percentage > 0) {
            $form->grade_2 = $new_price * ($grade_2_percentage / 10); // 按比例计算折扣，8折即乘以0.8
        }
        if (!empty($grade_3_percentage) && $grade_3_percentage > 0) {
            $form->grade_3 = $new_price * ($grade_3_percentage / 10); // 按比例计算折扣，9折即乘以0.9
        }
    }
