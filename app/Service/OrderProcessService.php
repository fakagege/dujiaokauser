 ...
 * 根据用户等级获取商品价格
 *
 * @param User|null $user 用户实例
 * @param Goods $goods 商品实例
 * @return float
 */
private function getPriceBasedOnUserGrade(?\App\Models\User $user, Goods $goods): float
{
    // 如果用户未登录或等级为 0，则使用商品的实际价格
    if (!$user || $user->grade === 0) {
        return $goods->actual_price;
    }

    // 根据用户等级设置价格
    switch ($user->grade) {
        case 1:
            return $goods->grade_1 !== null && $goods->grade_1 > 0 ? $goods->grade_1 : $goods->actual_price;
        case 2:
            return $goods->grade_2 !== null && $goods->grade_2 > 0 ? $goods->grade_2 : $goods->actual_price;
        case 3:
            return $goods->grade_3 !== null && $goods->grade_3 > 0 ? $goods->grade_3 : $goods->actual_price;
        default:
            return $goods->actual_price;
    }
}
...
private function calculateTheTotalPrice(): float
{
    ...
    // 使用用户等级价格，如果等级为0，则使用商品的实际价格
    $price = $this->getPriceBasedOnUserGrade($user, $this->goods);
    ...
    public function createOrder(): Order
{
    try {
        ...
        $order->goods_price = $price; // 使用等级决定的价格
