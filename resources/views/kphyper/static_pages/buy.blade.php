...
@endif
           ...
           <span class="buy-price"> {{ $actual_price }} {{ dujiaoka_config_get('global_currency') }}</span>
               ...
               @if($preselection >= 0 && !empty($selectable))
                 @if(collect($selectable)->contains(function($carmi) {
        return !empty($carmi['info']);
          }))
        <div class="form-group">
            <div class="buy">
                <div>
                    <label class="col-form-label">{{ __('dujiaoka.preselection') }} 
                        <b>{{ $preselection }} {{ dujiaoka_config_get('global_currency') }}</b> 
...
<div class="card card-body buy-product">
    {!! $description !!}
    <br>    <br>    <br> 
    @if(dujiaoka_config_get('is_open_spwz') == \App\Models\BaseModel::STATUS_OPEN)
        @if(!empty($articles))
            @php
                // 从文章列表中随机选择一篇文章
                $randomArticle = $articles->shuffle()->first();
            @endphp
            <div class="random-article">
                <h5 style="color: red;">推荐文章</h5>
                <p>
                    <a href="{{ url('article/' . (!empty($randomArticle['link']) ? $randomArticle['link'] : $randomArticle['id'])) }}.html">
                        <img src="{{ picture_ulr($randomArticle['picture']) }}" style="width: 30px; height: 30px; margin-right: 10px;" class="img-thumbnail">
                        {{ $randomArticle['title'] }}
                    </a>
                </p>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    var carmiCarousel = document.getElementById('carmiCarousel');

    if (carmiCarousel) {
        // 查找轮播项中是否存在有效的 carmi_id
        var carmiIdInputs = carmiCarousel.querySelectorAll('input[name="carmi_id"]');

        if (carmiIdInputs.length === 0) {
            // 如果没有 carmi_id，隐藏箭头
            var prevArrow = carmiCarousel.querySelector('.carousel-control-prev');
            var nextArrow = carmiCarousel.querySelector('.carousel-control-next');
